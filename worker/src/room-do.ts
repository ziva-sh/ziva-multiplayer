...
import { peerJoin, peerLeave, tagSender, welcome } from "./proto/v1";
...
export class RoomDO extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    // Notify every already-connected peer that someone new joined. The new
    // peer itself doesn't get this — they learn their own id from `welcome`.
    const joinFrame = peerJoin(peerId);
  ...
  async webSocketClose(
    ...
    if (meta) {
      ...
      const leaveFrame = peerLeave(meta.peerId);
      for (const peer of this.ctx.getWebSockets()) {
        if (peer === ws) continue;
        try {
